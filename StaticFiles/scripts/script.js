...
let transform = (($( window ).width()/ $( window ).height()) < 1.3) || isMobile ;
let graphString =  "";
...
function onLoad(){
  ...
  if($("#dummy").length != 0){
    ...
    for(let i = 0; i < collegeBlacks.length; i++){
    }
    let mainWidth =  $( window ).width() / 4;
    $(".graphDiv").css("min-width" , mainWidth+"px");
        ...
        $("#recs").append(tempString);
            ...
            if(isNaN(parseInt(collegeACTS[i]))){
              ...
              if(useract > act1){
                ...
                $("#actimage"+(i+1)).attr("src","StaticFiles/pictures/greenlight.png");
                ...
                $("#actimage"+(i+1)).attr("src","StaticFiles/pictures/redlight.png");
                $("#actimage"+(i+1)).css("max-width", "48%");
              }
              else{
                $("#yesornoact"+(i+1)).text("You score about the same to the average student at this school!");
                $("#actimage"+(i+1)).attr("src","StaticFiles/pictures/yellowlight.png");
                $("#actimage"+(i+1)).css("max-width", "48%");
              }
            }
            ...
            if(isNaN(parseInt(satcumulative1))){
              ...
              if(usersat > satcumulative1){
                ...
                $("#satimage"+(i+1)).attr("src","StaticFiles/pictures/greenlight.png");
                ...
                $("#satimage"+(i+1)).attr("src","StaticFiles/pictures/redlight.png");
                $("#satimage"+(i+1)).css("max-width", "48%");
              }
              else{
                $("#yesornosat"+(i+1)).text("You score about the same to the average student at this school!");
                $("#satimage"+(i+1)).attr("src","StaticFiles/pictures/yellowlight.png");
                $("#satimage"+(i+1)).css("max-width", "48%");
              }
            }
      }
...
$(document).ready(() => {
  if(isMobile){
    $(".graphDiv").css({"min-width" : mainWidth + "px", "min-height" : "20vh"});
    $(".bigButtonDiv").css({"min-width" : (mainWidth * .79) + "px"});
    $(".stats").css({"min-width" : mainWidth + "px"});
    $(".columnitems").css({"width" : "80%"});
    $(".collegewebsitebutton").css({"font-size" : "1.2vh"});
...
"<div class = 'box'>" +
  "<div class = 'stats'>"+
    "<div style='width: 50%;' class='scoresfor"+num+"'>" +
        "<p class='describer'>" +
          "Average ACT score:" +
        "</p>" +
...
"<div class = 'box'>"+
  ...
  "</div> </div>"+
"</div>"+
"<div style = 'width: 80%; margin-right: auto; margin-left: auto;' class = 'box'>"+
  "<div style='width: 50%; text-align: center;' class = 'bigButtonDiv'>"+
    "<form action='https://" + LinkOfCollege + "'>"+
      "<input type='submit' value='Go to " + collegeName + "\'s website' class = 'collegewebsitebutton'/>"+
    "</form>"+
  "</div style='width: 50%;'>"+
  "<div style='width: 50%;' class = 'bigButtonDiv'>"+
...
let url1;
  ...
  if(state.length == 0){
    ...
    arrayToString2(collegeObjs);
    // $("#collegecost").val(arrayToString2(collegeprices));
    // $("#colleges").val(arrayToString(collegeNames));
    // $("#collegestates").val(arrayToString(collegestates));
    // $("#collegecities").val(arrayToString(collegecities));
    // $("#collegetemperatures").val(arrayToString(collegetemps));
    // $("#collegeweather").val(arrayToString(collegeweathers));
    // $("#collegeact").val(arrayToString(collegeACTS));
    // $("#collegesatreading").val(arrayToString(collegeReadingSAT));
    // $("#collegesatmath").val(arrayToString(collegeMathSAT));
    // $("#collegeblackpop").val(arrayToString(collegeBlacks));
    // $("#collegewhitepop").val(arrayToString(collegeWhites));
    // $("#collegeasianpop").val(arrayToString(collegeAsians));
    // $("#collegehispanicpop").val(arrayToString(collegeHispanics));
    // $("#useract").val(arrayToString(useract));
    // $("#usersat").val(arrayToString(usersat));
  }
  ...
  // url2 = "https://api.openweathermap.org/data/2.5/weather?q="+collegeObjs[0].city+",USA&appid=01e2ab091c2e3a0ab1eaca9b7e15f159";
...
$( window ).resize(function() {
  if(($( window ).height()/ $( window ).width()) > 1.5 || isMobile){
    ...
    $(".graphDiv").css({"min-width" : "400px", "min-height" : "50vh"});
    $(".bigButtonDiv").css({"min-width" : "400px"});
    $(".stats").css({"min-width" : "500px", });
    $(".columnitems").css({"width" : "100%"});
    $(".collegewebsitebutton").css({"font-size" : "16px"});
    $(".c").css("font-size", "5vw");
